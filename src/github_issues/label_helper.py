...
class LabelHelper:
    ...
    def get_label_from_id_name(self, label_info_dict, key_name=None, label_prefix='', non_formatted=False):
        ...
        if label_info_dict.get('multiple'):
            return None

        if 'id' in label_info_dict and 'name' in label_info_dict:
            if 'value' in label_info_dict and label_info_dict['value'] in ['', [], '0', 0, None, 'Normal']:
                return None

            if key_name and label_info_dict['name'] in ['', [], '0', 0, None, 'Normal']:
                return None

            if label_info_dict['name'] in ['Triaged', 'Groomed', 'Bugzillas']:
                return label_info_dict['name']

            if non_formatted and 'value' in label_info_dict:
                return '%s: %s' % (label_info_dict['name'], label_info_dict['value'])
            ...
            if key_name == 'status':
                return label_info_dict['name'].title()
    ...
    def get_label_names(self, redmine_issue_dict, non_formatted=False):
        ...
        label_names = ["Migrated from Redmine"]

        # Add status
        status_label_name = self.get_label_from_id_name(redmine_issue_dict, 'status', '', non_formatted)
        if status_label_name:
            label_names.append(status_label_name.title())

        # Add tracker
        tracker_label_name = self.get_label_from_id_name(redmine_issue_dict, 'tracker', '', non_formatted)
        ...
        if custom_fields and len(custom_fields) > 0:
            for cf_dict in custom_fields:
                component_label_name = self.get_label_from_id_name(cf_dict, None, '', non_formatted)
                if component_label_name:
                    label_names.append(component_label_name)
                # Add tags
                if cf_dict.get('multiple', False):
                    for value in cf_dict.get('value', []):
